...
| `skills/ceratops-credit-savings-analysis/scripts/credit_analysis/report_bookkeeping.py` | Owns result-shape validation, surface ordering, temporary-control links, category consolidation, and reviewer-record preservation. Final results retain complete original risks, control reviews, and category assessments in controller-generated `source_risks` and `source_reviews`, checked against accepted reviewer records. Copied category checklists aggregate applicability across reviewed portions without discarding differing assessments. Reports display every distinct risk uncertainty. The controller revalidates saved final output before enforcing limits on new attempts and selects the highest-priority complete audit window that fits the reserved review slot. |
