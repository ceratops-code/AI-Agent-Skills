...
- Configure default-branch protection with real required checks, strict status checks, PR flow, `required_approving_review_count: 1`, a bypass allowance for the authenticated maintainer account, stale review dismissal, conversation resolution, admin enforcement, no force pushes, and no deletions when available at no extra cost.
